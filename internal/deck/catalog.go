...
func LoadFS(fsys fs.FS) (*Catalog, error) {
	...
	for _, entry := range deckEntries {
		...
		for _, cardID := range deck.BlackIDs {
			if _, ok := c.BlackCards[cardID]; !ok {
				return nil, fmt.Errorf("%s: unknown black card id %q", dir, cardID)
			}
		}
		for _, cardID := range deck.WhiteIDs {
			if _, ok := c.WhiteCards[cardID]; !ok {
...
func loadDeckDir(fsys fs.FS, dir string) (*Deck, error) {
	metaPath := path.Join(dir, "deck.json")
	raw, err := fs.ReadFile(fsys, metaPath)
	if err != nil {
		return nil, fmt.Errorf("%s: read deck metadata: %w", metaPath, err)
	}
	var meta DeckMetadata
	if err := json.Unmarshal(raw, &meta); err != nil {
		return nil, fmt.Errorf("%s: decode deck metadata: %w", metaPath, err)
	}
	if meta.ID == "" || strings.TrimSpace(meta.Name) == "" {
		return nil, fmt.Errorf("%s: %w: deck missing id or name", metaPath, ErrInvalidDeck)
	}
	...
	if len(blackIDs) == 0 && len(whiteIDs) == 0 {
		return nil, fmt.Errorf("%s: %w: deck must include at least one card", dir, ErrInvalidDeck)
	}
	return &Deck{
		DeckMetadata: meta,
...
func (c *Catalog) UnionCards(deckIDs []string) (black []*BlackCard, white []*WhiteCard, err error) {
	...
	for id := range blackSet {
		black = append(black, c.BlackCards[id])
	}
...
func (c *Catalog) unionSet(deckIDs []string) (map[string]struct{}, map[string]struct{}, error) {
	if c == nil {
		return nil, nil, nil
	}
	...
	for _, deckID := range uniqueSorted(deckIDs) {
		...
		for _, id := range deck.BlackIDs {
			blackSet[id] = struct{}{}
		}
		for _, id := range deck.WhiteIDs {
			whiteSet[id] = struct{}{}
		}
	}
